...
10. **Uncertainty: flag, do not fabricate.** When a fix needs a model/API fact the checklist and loaded family file lack, or the API may have drifted, do not invent it. Surface a deployer-verify item in Key Changes with your interim assumption stated; for Gemini targets, recommend the `gemini-interactions-api` skill (if available) or a docs MCP search.
