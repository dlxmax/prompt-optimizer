...
**Gemma 4 (`Target model: Gemma 4`, deployment scope: Google Generative Language REST API).** T=1.0 recommended (not T=0). 26B A4B has a double tool-call bug; use 12B or 27B dense for tool-calling workflows (both variants are equivalent for thinking and `responseSchema` mechanics). **`generationConfig.responseSchema` is the primary deployment lever, not a Tier 2 option.** JSON adherence via prompt instructions is the primary weakness; the schema (OpenAPI-3.0 subset) fixes structure AND suppresses thinking entirely AND produces a measured ~30 to 40x wall-clock speedup on short outputs (May 12, 2026 burst-rewrite benchmark: 67s/call median dropped to 1 to 2s/call, MALFORMED rate dropped from baseline to 0%). Parse responses with `json.JSONDecoder().raw_decode()` not `json.loads()` (Gemma 4 occasionally appends trailing text after valid JSON, ~1 in 12 calls). **Thinking is always on** and surfaces as `parts[].thought = true` (not as `<|channel>` text markers); `usageMetadata.thoughtsTokenCount` exposes cost. Filter `parts[].thought` client-side, or use `responseSchema` which collapses output to a single non-thought part. Thinking cannot be disabled: `thinkingLevel: "low"`/`"off"` and `thinkingBudget: 0` all return 400; `thinkingLevel: "high"` is silently accepted but no-op; do not place `<|think|>` in `systemInstruction` (no-op + elevates the transient 500 rate). **`maxOutputTokens` is a safety ceiling, not a thinking-budget cap:** Gemma 4 expands thinking to fill whatever budget is set (256 cap → ~310 thinking tokens, 1024 cap → ~1150, 2048 → more); lowering the cap converts MALFORMED_RESPONSE timeouts into MAX_TOKENS fast-fails (cheaper failure) but does NOT increase success rate. Use `responseSchema` for thinking control; set `maxOutputTokens` generously. Classify retries by failure mode: 500/503 transients retry with same params (3 attempts, flat 1s wait, ~20% baseline rate); MALFORMED_RESPONSE retries with parameter changes, not same-params repeat. Strong instruction-following increases injection risk; delimiter blocks with explicit data-only instructions are required. (Probes May 6, 2026, 28 calls; supplemented May 12, 2026, 72-call burst-rewrite benchmark.)
