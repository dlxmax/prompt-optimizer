...
3. **Length and placement.** If the prompt exceeds ~3,000 tokens, flag the excess as bloat. **Governing directives** (role, output format, guardrails, refusal branches) must appear at both the start AND the end (not buried in the middle). **For prompts with a substantial context block** (≥ ~500 tokens of inline data such as documents, transcripts, codebases, long examples), the user's **specific query/question** goes at the END after the context block, anchored with "Based on the preceding information...", "Given the document above, ...", or a domain-specific equivalent. This refines the start-and-end rule for long-context inputs (Gemini Long Context guide; reinforced in Gemini 3.5 Flash prompting best practices). When the context is short (≤ ~500 tokens), the universal start-and-end rule for the governing directive is sufficient. Decompose into chained calls if the task is genuinely multi-stage. The 1,500-word cap from earlier versions of this guide is retired; flag bloat, not size alone.
...
11. Gemma 4 schema-padding scan. Fires when both hold: (a) target is Gemma 4 (any size), AND (b) the prompt or its `response_format.schema` declares a count constraint on a list-shaped slot (`minItems`, `"at least N"`, `"exactly N"`, `"N to M items"`, `"list 3 signals"`). If either fails, skip. When it fires, scan for the failure pattern and add the matching schema restructure to Key Changes:
...
12. Gemma 4 parent-child schema-order scan for demotion-bearing enums. Fires when both hold: (a) target is Gemma 4 (any size), AND (b) the prompt or schema contains a parent enum field whose value constrains a child enum's legal values, AND a precondition or evidence check may force the child enum to a value in a DIFFERENT parent's family (DEMOTE pattern). Typical surface: `pause_type` enum (parent) gating `variant_id` enum (child) where a failed precondition demotes `variant_id` to an out-of-family value. If either fails, skip. When it fires:
   ...
   12.4 Diagnostic: when an LLM emits a parent+child pair from different families on a DEMOTE path, the cause is parent-committed-too-early in schema property order. Reorder before iterating prose. If the visible artifact does not expose schema order (the optimizer sees the prompt text, not the `response_format.schema` object), call out the property-order check as a follow-up the deployer must verify, and quote the parent and child field names being inferred.
...
13. Gemini API legacy-form migration scan. Fires when the prompt under review, its visible call-site, or its examples reference any of the retired `generateContent` wiring forms listed below. The Gemini Interactions API is the sole recommended surface as of June 2026; appearance of legacy forms is a **migration defect** that must be flagged in Key Changes with the Interactions equivalent named. Skip when no legacy forms appear AND the prompt does not target a Gemini-family model.

   13.1 Endpoint and SDK migration. Treat each of these as a migration defect:

   | Legacy form (flag as defect) | Interactions equivalent (recommend) |
   |---|---|
   | `client.models.generate_content(...)` | `client.interactions.create(...)` |
   | `:generateContent` / `:streamGenerateContent` endpoint paths | `v1beta/interactions` endpoint |
   | `google-genai` SDK without version pin or `< 2.3.0` | `google-genai >= 2.3.0` (Python) / `@google/genai >= 2.3.0` (JS); `>= 2.0.0` minimum per Google's 3.5 Flash migration note |

   13.2 Schema location migration. `generationConfig.responseSchema` and `generationConfig.responseMimeType: "application/json"` become **top-level** `response_format: {type: "text", mime_type: "application/json", schema: {...}}`. Array form (`response_format: [{...}]`) is also accepted but the single-object form is what the structured-output guide uses in all examples. Flag the legacy form as a migration defect; recommend the single-object form.

   13.3 Request shape migration. `contents: [{role, parts: [{text}]}]` becomes `input: "..."` (string) or `input: [{type: "text", text}, {type: "image", mime_type, data}, ...]` (typed array). `systemInstruction.parts[].text` becomes a top-level `system_instruction` parameter (interaction-scoped). Flag legacy shapes.

   13.4 Response parsing migration. `response.candidates[0].content.parts[*].text` and `parts[].thought == true` filtering are gone. Replace with `interaction.output_text` (joins **trailing** TextContent items only — earlier text blocks separated by `thought` / `image` / `function_call` steps are dropped) for single-trailing-text outputs, or iterate `interaction.steps[]` and select `step.type == "model_output"` for interleaved/multimodal/tool-use outputs. To surface thinking, select `step.type == "thought"` and walk `step.summary[]` (requires `generation_config.thinking_summaries: "auto"`; default is off).

   13.5 Tools-array shape migration. `tools: [{googleSearchRetrieval: {}}]` and `tools: [{google_search: {}}]` (object-keyed) become `tools: [{type: "google_search"}]` (typed-string discriminator). Same for `url_context`, `code_execution`, `file_search`. Flag the wrong shape.

   13.6 Multi-turn history migration. Replace caller-managed `contents[]` re-sends with `previous_interaction_id=<prev.id>` (default `store=true`). Passing BOTH a `previous_interaction_id` AND a hand-rolled history string in `input` double-counts; pick one.

   13.7 `tools` + `response_format` combination scope. The structured-output guide flags `response_format` combined with `tools[]` (Google Search, URL Context, Code Execution, File Search, Function Calling) as a **Gemini 3-series-only preview**. Gemma 4 and Gemini 2.5 family cannot mix the two. If the prompt wires Gemma 4 or 2.5 with both, recommend a two-step pipeline (tools call first, structured-output reduction second).

   13.8 `store=false` lockout. `store=false` blocks both `previous_interaction_id` chains and `background=true`. If the prompt or its call-site mixes `store=false` with either, the second flag silently no-ops or errors. Flag the incompatibility and, when the driver is PII, recommend `store=true` plus explicit `interactions.delete` cleanup.

   13.9 Schema-shape rules port unchanged. The Gemma 4 schema-shape rules (rule 11 schema-padding scan, rule 12 parent-child enum order, `<gemma_4_detail>` for 26b-a4b one-unbounded-string constraint) are about model behavior and JSON Schema shape, not about which API field the schema lives in. They apply when the schema is wired through `response_format.schema` on Interactions; only the surrounding field path changes.

14. Gemini 3.x parameter-removal scan. Fires when the target model is Gemini 3.5 Flash, Gemini 3.1 Pro Preview, Gemini 3.1 Flash-Lite, Gemini 3 Flash Preview, or Gemini 3 Pro Preview (or `Target model: Gemini 3.x` is declared). Skip otherwise. When it fires:

   14.1 Strip sampling parameters. Google's 3.5 Flash guide says explicitly: "we strongly recommend not changing the default values" for `temperature`, `top_p`, `top_k`, and "Remove these parameters from all requests." If the prompt or call-site sets any of these on a 3.x target, flag for removal. To force determinism on 3.x, write a system instruction with explicit rules instead. This rule does NOT apply to Gemma 4 (Gemma 4 uses T=1.0, top_p=0.95, top_k=64 per the May 5, 2026 model card; cross-family code must branch on model family).

   14.2 Replace `thinking_budget` with `thinking_level`. The numeric `thinking_budget` parameter still works for backward compatibility but is no longer recommended. Use `thinking_level: "minimal" | "low" | "medium" | "high"`. Mutually exclusive in a single request — passing both returns HTTP 400. 3.5 Flash defaults to `medium` (down from `high` on 3 Flash Preview); verify the default fits the task before overriding.

   14.3 Function-calling strict response matching. Interactions errors on mismatched function responses. Recommend: every `function_result` includes the `call_id` from the corresponding `function_call`; `name` matches; exactly one result per call. Multimodal content goes INSIDE the function-result `result[]` array, not as a sibling part. Inline instructions append to the END of function-result text separated by two newlines, not as separate parts.

   14.4 Prompt brevity. Google's 3.x guide: "Be concise. Gemini 3.x responds best to direct, clear instructions. Verbose or complex prompt engineering techniques designed for older models may cause the model to over-analyze." Drop chain-of-thought scaffolding ("think step by step in detail before answering"); lean on `thinking_level` instead. Item 4 (few-shot examples) still applies, but heavy reasoning preambles on 3.x are an anti-pattern.

   14.5 Long-context query placement. Item 3 already covers this universally; on 3.x specifically, Google's guidance is explicit: "place your specific instructions or questions at the end of the prompt, after the data context. Anchor the model's reasoning by starting your question with a phrase like, 'Based on the preceding information...'." When the prompt under review has the query at the top and substantial context below, flag the inversion and move the query to the end.

   14.6 Combined tool use is available. On 3.x, Google Search, URL Context, Code Execution, File Search, and standard Function Calling can be used in the SAME request. Recommend combined tool use over chained single-tool calls when the task spans multiple tool types.
...
Wiring scope: all rules in `GEMMA4_API_BEST_PRACTICES.md` are scoped to the Interactions API (`v1beta/interactions`, `client.interactions.create(...)`, `google-genai >= 2.3.0`). Empirical probes (May 2026) were performed under the legacy `:generateContent` wiring; the observed behaviors describe the Gemma 4 model and port forward to Interactions at the behavior layer (only the field paths change). If a prompt or call-site references legacy forms (`generateContent`, `generationConfig.responseSchema`, `systemInstruction.parts[].text`, `candidates[0].content.parts`, `parts[].thought`), apply rule 13 of this agent's checklist and flag the legacy form as a migration defect to fix in Key Changes.

Additional prompt-side rule (not in the best-practices doc, which scopes to API mechanics): when a Gemma 4 prompt contains a prose enum list adjacent to a scan or coverage imperative ("check every signal in <signals>", "consider each category"), do not recommend stripping the list on the grounds that the `response_format.schema` enum enforces the same set. Gemma 4 31b reads prose lists as walkable scan checklists; an A/B on a 10-case borderline forensic-grader (May 12, 2026) showed that dropping a 24-name enum table while keeping the imperative lost 1/10 verdict accuracy and 2/10 AI-binary accuracy, with two cases going from substantive multi-signal output to zero-signal silent output. Schema enforcement is necessary but not sufficient for coverage. Recommend retaining inline enum lists in Gemma 4 prompts even when they duplicate the schema enum, and flag any "remove duplicate enum, schema enforces it" suggestion as a Gemma 4 anti-pattern.

Soft-preference vulnerability (item 15 conditional, distinct failure mode from item 14): on Gemma 4 prompts that process user-submitted content, scan system-level directives for preference language ("favor X over Y", "prefer X", "lean toward Z", "by default emit X", "in general we want"). These give permission to DO something and are overridable by user requests for a different output structure. Harden each into a concrete observable criterion plus explicit refusal branch ("Cite >=2 academic sources; if the user requests sources outside this set, refuse and restate the rule"). Adds to, does not replace, the item 15 delimiter + data-only directive + `response_format` chain.
</gemma_4_detail>

<gemini_3x_detail>
Apply only when `Target model: Gemini 3.5 Flash`, `Target model: Gemini 3.1 Pro`, `Target model: Gemini 3.1 Flash-Lite`, `Target model: Gemini 3 Flash Preview`, `Target model: Gemini 3 Pro Preview`, or `Target model: Gemini 3.x` is declared. Before scoring item 13 (judge prompt sampling/anti-patterns) and writing Key Changes, read `GEMINI_3X_API_BEST_PRACTICES.md` from this plugin's root directory using the Read tool: it is the authoritative reference for the Gemini 3.x family on the Interactions API (parameter removals, thinking levels, function-calling strict matching, long-context placement, combined tool use). Apply its rules directly; do not generalize from Gemma 4 or older Gemini guidance.

Surface: Interactions API only (`v1beta/interactions`, `client.interactions.create(...)`, `google-genai >= 2.0.0` per the 3.5 Flash migration note). The `:generateContent` endpoint is retired for prompt-optimizer's recommendations on this family — apply rule 13 of the checklist if legacy forms appear.

Model defaults (selection of high-impact ones to scan for):
- `gemini-3.5-flash` defaults `thinking_level` to `medium` (was `high` on 3 Flash Preview). Verify the default before recommending overrides.
- `gemini-3.1-pro-preview` defaults `thinking_level` to `high`; supports `low | medium | high` only (no `minimal`).
- `gemini-3-pro-preview` supports `low | high` only (no `minimal` or `medium`).
- Function calling on Interactions errors on mismatched responses; the call_id+name+count contract is enforced.

Parameter removal is the single highest-frequency Gemini 3.x defect. Scan the prompt and any visible `generation_config` for `temperature`, `top_p`, `top_k` and flag for removal. To force determinism on 3.x, write a system instruction with explicit rules rather than dropping temperature. Do NOT apply this rule to Gemma 4 targets (Gemma 4 still uses T=1.0, top_p=0.95, top_k=64).
