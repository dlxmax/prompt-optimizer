...
17. Tie-break direction is policy, not a determinism mechanic. Fires when the revised prompt adds or proposes a determinism scaffold for band selection, rubric scoring, grade thresholds, or any closed-set choice with potential exact-boundary ties.

   17.1. Determinism requires identical evidence to resolve the same way every time; direction does not. UP and DOWN are equally deterministic. Treat tie-break direction as a separate, grade-affecting policy decision. Do not bundle a directional default ("on a tie take the lower band", "borderline resolves DOWN", "default to lower on any doubt") into a determinism fix as if it were the obviously-correct mechanic.

   17.2. Distinguish a true tie from doubt. True tie = exact boundary between two fully-fit bands (every clause satisfied for both bands). Doubt = a clause not clearly satisfied for one band; the AND-gate (entry condition that admits a band only when every clause is met) already resolves doubt by not admitting the band. Conflating them ("default to lower band on any doubt") double-counts downward pressure: clause strictness handles doubt; the directional tie-break applies only to genuine exact ties.

   17.3. Surface the direction in Key Changes; do not assume. When adding an exact-tie rule, name the direction choice explicitly: "Ties currently have no stated direction. I set exact-boundary ties to resolve UP / DOWN; confirm this matches the scoring convention." Before recommending a default, scan the source codebase, sibling prompts, or the prompt-under-review's surrounding context for an established rounding/tie convention ("ties favor the student", "round midpoints UP", "exact halves round to even"); match it. When the convention is not detectable, flag the direction as a deployer-side decision in Key Changes rather than picking silently.

   17.4. Negative scan targets in revised prompt text: "on any doubt take the lower band", "borderline resolves down", "default to the lower band", "in cases of uncertainty take the lower", "lean conservative on ties", "exact-midpoint scores resolve DOWN", and the same patterns with UP smuggled in without justification. Each conflates strictness (clause satisfaction) with directional policy. Replace with the two-rule pattern: (a) AND-gate clause strictness in the rubric body, (b) a single explicit directional rule for genuine exact ties, surfaced in Key Changes for caller confirmation.
</rules>
