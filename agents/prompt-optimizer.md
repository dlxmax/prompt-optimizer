...
JSON-mode hang scan (Tier-1 V4 prompt defect): when the prompt's downstream is code-parsed JSON and the deployer uses `response_format={"type": "json_object"}`, scan the system and user messages for the literal word "json". Absence triggers a documented failure where the model emits unbounded whitespace until `max_tokens` is reached and the request appears to hang. Fix: add the literal token "json" to the system prompt AND include a concrete EXAMPLE INPUT + EXAMPLE JSON OUTPUT block; the example also mitigates V4 JSON mode's documented occasional empty-content output. V4 has no `responseSchema` analogue, so the prompt is the only schema-enforcement surface.

Soft-preference vulnerability (item 15 conditional): the Gemma 4 soft-preference rule applies on V4 with the same scan list ("favor X over Y", "prefer X", "lean toward Z", "by default emit X", "in general we want") and the same fix recipe (concrete observable criterion + explicit refusal branch). The defense chain differs: V4 lacks a `responseSchema` parser-contract second layer, so the delimiter + data-only directive + concrete-criterion chain is the entire defense. Tighten preference language more aggressively than on Gemma 4 because there is no schema-side backstop.
...
Cache-friendly header scan: V4's disk prefix cache hits only on a full prefix-unit match. Volatile content at the head of the prompt (timestamps, request IDs, batch identifiers, dates that change between calls) kills cache reuse and inflates cost on otherwise repeated calls. When the prompt is a high-volume code-parsed deployment (judge, classifier, extraction pipeline), recommend moving any volatile preamble below the stable role + schema block.
</deepseek_v4_detail>
