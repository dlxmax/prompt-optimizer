...
10. Avoid 26B A4B for tool-calling workflows (double tool-call bug). The 26B A4B and 31B variants behave identically for thinking control and for the single-STRING `responseSchema` mechanics in item 2, but they diverge on multi-STRING schemas (item 11).
11. **`responseSchema` with two or more unbounded free-text STRING properties per OBJECT fails on `gemma-4-26b-a4b-it` (probe-verified May 12, 2026).** Failure modes split between deterministic bigram/trigram loops to `MAX_TOKENS` (e.g., `-classification-classification-...`, `0.0.0.0.0...`) and degenerate empty `" [] ```"` output. The same schema passes cleanly on `gemma-4-31b-it`. Per-field `ARRAY[STRING]` wrapping does NOT rescue this: the loop migrates to whichever sibling unbounded STRING the model reaches first, and ALL-fields-ARRAY shifts the failure to HTTP 500 / empty array. Verified workarounds when 26b-a4b is in the fallback chain: (a) caller-side `responseSchema` skip on the model-name match for 26b-a4b, or (b) explicit prompt-level bounding ("one short sentence") plus a worked stop-example on every unbounded STRING field. Schemas with exactly one unbounded STRING per OBJECT inside a top-level `ARRAY[OBJECT]` container (the `audit[].reason` shape) have held in production. The full data table and probe scripts are documented in [`GEMMA4_26B_A4B_SCHEMA_RUNAWAY.md`](../GEMMA4_26B_A4B_SCHEMA_RUNAWAY.md); the consolidated mechanical fix list is in [`GEMMA4_FIXES.md`](../GEMMA4_FIXES.md). When the prompt under review targets a fallback chain that includes 26b-a4b and the schema has multiple unbounded STRING properties per OBJECT, the revised prompt's Key Changes must call out which workaround the caller will adopt; do not recommend `ARRAY[STRING]` wrapping as the fix.
