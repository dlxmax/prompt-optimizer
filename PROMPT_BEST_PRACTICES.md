...
| Via Google's REST API (`generativelanguage.googleapis.com`), Gemma 4's `<\|think\|>` thinking-channel mechanism does not surface — `<\|channel\|>thought>` markers never appear in responses. Asking Gemma to "first think then answer" via `<thinking>...</thinking>` XML scaffolds causes runaway reasoning until the token cap, with no parseable output. | Empirical REST API probes against `gemma-4-31b-it` and `gemma-4-26b-a4b-it`, May 2026 | Do not request thinking via channel tokens or XML scaffolds. If reasoning helps, request a bounded reasoning field inside `responseSchema` (e.g., a `notes` string with a character cap). Bound `maxOutputTokens` to 1024–2048 on prompt-only output paths. |
...
**Gemma 4 note.** When targeting Gemma 4 via Google's Generative Language REST API (`generativelanguage.googleapis.com/v1beta/models/<model>:streamGenerateContent`), the API layer handles conversation boundaries — callers send `contents: [{role, parts}]` arrays and never write turn-control tokens. Internal control tokens (`<|turn>` / `<turn|>`, `<|think|>`, `<|channel|>`) are not surfaced to or from the REST endpoint. The XML-style semantic tags above (`<role>`, `<instructions>`, `<context>`, etc.) are the right tool for structuring prompt content and remain fully effective.
...
| Gemma 4 via Google REST API with `responseSchema` containing a bounded `<reasoning>` field before `<verdict>` | Reliable — schema-enforced inline reasoning is the working substitute for the `<\|think\|>` thinking channel, which does not surface on the REST endpoint (see Section 5.8). Free-form `<thinking>` XML scaffolds do not work and cause runaway reasoning. |
...
**Gemma 4 note.** Via Google's REST API, the `<|think|>` thinking-channel mechanism does not surface and `<thinking>...</thinking>` XML scaffolds cause runaway reasoning rather than self-correction (see Section 5.8). For high-stakes judge calls, do not attempt to enable thinking; instead, attach a `responseSchema` with `<reasoning>` (bounded length) and `<verdict>` fields. The schema enforces the structured self-correction step that an XML scaffold cannot reliably elicit on this endpoint.
...
**Thinking mode does not surface via REST API.** Empirical probes (May 2026) against `gemma-4-31b-it` and `gemma-4-26b-a4b-it` on `generativelanguage.googleapis.com/v1beta/models/<model>:streamGenerateContent` show that the `<|think|>` thinking-channel mechanism documented for the model is not exposed by the hosted REST endpoint. With and without `<|think|>` placed at the start of the system instruction, responses contained zero `<|channel`, `<channel`, `<thinking`, `thought`, or `reasoning` markers — only freeform reasoning text running until the token cap. Asking Gemma to "first think, then answer" via `<thinking>...</thinking>` XML scaffolding makes this worse: the model ignores the closing tag and re-derives indefinitely, producing no parseable answer. For judge prompts:

- Do not request thinking via `<|think|>` channel tokens or `<thinking>` XML scaffolds
- Do not write multi-turn history-stripping logic that searches for channel markers — they do not appear in REST responses
- If reasoning genuinely improves quality, request a **bounded** reasoning field inside the structured output (a `<reasoning>` field in `responseSchema` with a character cap), not a free-form preamble
- Bound `maxOutputTokens` to 1024–2048 on prompt-only output paths so runaway reasoning is bounded by the cap rather than by prompt instructions
- Modulate reasoning depth via the schema field's `description` ("brief justification, max 200 chars" vs. "detailed criterion-by-criterion analysis") rather than via prompt flags

**JSON and structured output: use `responseSchema`.** Gemma 4's primary documented weakness is JSON format adherence when format is requested via prompt instructions alone. The fix on this endpoint is `generationConfig.responseSchema` (OpenAPI-3.0 subset). Probe verification: prompt-only JSON request returned 8333 chars freeform with no JSON; same task with `responseSchema` returned 70 chars clean JSON matching the schema, parseable on first try. Schema fields supported: `type` (`STRING` / `INTEGER` / `NUMBER` / `BOOLEAN` / `ARRAY` / `OBJECT`), `properties`, `required`, `items`, `enum`. The `description` field on each property is read by the model and acts as an in-schema instruction. Drop these from the prompt body when `responseSchema` is in use: `<thinking>` blocks, "Output only a raw JSON array" or "no markdown fences" instructions, field-by-field "must have these keys" lists, output-format examples in the prompt body. Duplicating the schema in the prompt only consumes context and gives Gemma more text to drift on. The `<reasoning>` and `<verdict>` field names in the judge template (Section 5.6) carry across to the schema and must be preserved.
...
These notes do not change the core checklist. They change the deployment pattern around it: same gates, rubric-first prompts, more samples, schema-enforced structured output via `responseSchema`, no reliance on the `<|think|>` thinking channel (which does not surface via REST), and variant selection that avoids 26B A4B for tool-calling.
...
13. **Judge prompt: sampling, model selection, and anti-patterns (conditional).** For high-stakes judge deployment: Is it run with N>=5 samples and majority vote (not single-pass; N=3 is insufficient)? Does it avoid debate-style (ChatEval) prompt structure — actively harmful at -158% worst-case consistency (Sage)? For Gemma 4 targets via Google REST API: use T=1.0 (not T=0; T=0 is not recommended and does not guarantee reproducibility). When output is parsed by code, is `generationConfig.responseSchema` used for structure rather than prompt-only format instructions? Are `<|think|>` channel tokens and `<thinking>...</thinking>` XML scaffolds avoided (they do not surface on this endpoint and cause runaway reasoning)? If reasoning is needed before the verdict, is it requested as a bounded `<reasoning>` field inside `responseSchema` rather than as a free-form preamble? Is `maxOutputTokens` bounded (1024–2048) on prompt-only output paths? Is the 26B A4B variant avoided for tool-calling judge workflows (documented double tool-call bug)? For highest-stakes ranking, does it use multi-model consensus (2-of-3 with Gemma 4 31B + Claude + GPT, achieving 88–96% human agreement) rather than a single judge model? See Section 5.8. N/A for low-stakes filtering and non-judge prompts.
