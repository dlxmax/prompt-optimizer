...
Every core rule about prompt content still applies; this file overrides where
the API mechanism the core rule names does not exist here. Harness behavior is
version-gated and perishable: floors stated where known; any fact this file
...
rule 8 filter 1's `ExitPlanMode` exception is therefore unreachable in one.

## 2. The markdown body IS the system prompt

Body + working directory is all the subagent gets. Not Claude Code's own system
prompt. Never rely on inherited harness instructions, tool descriptions, or
formatting conventions. Anything the prompt needs, the body states.
...
3.2. Abstention = a fixed literal (invariant 2, core rule 9). Free-form doubt is undetectable here for the same reason as on the API. A count in the contract ("list 3 signals", "give at least N") has no `minItems` to lower: the shortfall literal is the ONLY guard, and nothing enforces it. State the literal, and name the count check as parent-side.
3.3. Bounded verdicts = an explicit closed list in the body plus a restated "emit exactly one of these" line. Nothing enforces it; no grammar exists on this surface.

`GRADING_PIPELINE.md` artifact 3 and schema-review essentials, plus
`CLAUDE_STRUCTURED_OUTPUTS.md`, describe a response schema. They do not port
here: emit the tag contract instead and say so in Key Changes. Do not emit a
JSON schema the harness will never enforce.
...
schema retry, escalation re-call) runs in the parent, another agent, not your
pipeline. Specify each as a named parent-side requirement, or state that the
check is unenforced.
...
Three constraints on any output format emitted:

4.1. Never use a delimiter imitating a harness control tag. `<system-reminder>` and lookalikes get corrupted mid-token.
...
Scanning is not a permission boundary and does not judge content; parser hazard
only.
...
5.1. A text deliverable goes in the final message or is lost. The agent writes a file deliverable to disk; naming the path in the report is not delivery.
5.2. **The parent may summarize the report.** Verbatim preservation requires an instruction in the PARENT's prompt, which this file's target does not control. Any format whose value depends on exact bytes (score line, regex-extracted verdict) is a deployer-verify item: tell the caller to pass it through unsummarized.
...
- Variable per-run input (submission text, rubric, criterion) arrives in the invocation prompt string. The body describes the shape it arrives in and never templates it.
...
No `stop_reason` to branch on. An API error killing the subagent surfaces to
...
Retry, escalation, and abstain-on-failure policy cannot be implemented inside
the prompt. State each as a parent-side requirement. A truncated report is
indistinguishable from a complete one inside this agent's own output contract,
so a completeness marker as the LAST emitted line is the only in-band signal
available.
...
8.1. **`AskUserQuestion` never exists.** Never emit an instruction to ask the user a clarifying question, in any surface phrasing. Unstated policy choices go in the report as open deployer decisions, as the AUTHOR recipe prescribes.
...
intended and states that the env var can override it.
